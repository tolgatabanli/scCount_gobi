...
BAM_FILE="/home/t/tan/miniQuT3/data/star_run/Aligned.sortedByCoord.out.bam"
...
for k in $(seq 100 100 2000); do
	...
	(
		...
		"$BEDTOOLS_BIN" multicov -bed "${bed_file}.merged" -bams "$SPACERANGER_BAM_FILE" > "$out_file" 2>&1
	) &
	pid=$!
	pids+=("$pid")
